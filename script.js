...
function getAiReply(message) {
  ...
  if (text.includes('lenovo') || text.includes('legion')) {
    return 'The Lenovo Legion Y700 is available and is a strong tablet for gaming with a smooth display.';
  }
  if (text.includes('rog') || text.includes('asus') || text.includes('phone')) {
    return 'The ASUS ROG Phone 8 Pro is available and is a powerful gaming phone option.';
  }
  if (text.includes('headset')) {
    return 'Yes, we have a gaming headset available in the product section.';
  }
  ...
  if (text.includes('price') || text.includes('cost')) {
    return 'You can check prices directly in the product section. Ask me about any item if you want help choosing.';
  }
  if (text.includes('delivery')) {
    return 'After payment, go to the cart section and tap Proceed to Payment, then send proof of payment on WhatsApp.';
  }
  if (text.includes('payment') || text.includes('bank')) {
    return 'We currently accept payment through bank transfer using the account details in the cart section.';
  }
  return 'I can help with products, prices, payment, delivery, and recommendations.';
}
